...
- **Auto-snapshotted**: every Longhorn PVC is automatically covered by the daily `default`-group recurring snapshot job (retain 7) unless opted into a different group. This protects against accidental in-place data changes but not PVC/Volume deletion or node/disk failure. For the PVCs `scripts/backup-app-data.sh` covers (PostgreSQL, InfluxDB, n8n, Open WebUI, mosquitto, Grafana), node/disk failure and PVC deletion are covered by the manual app-data dumps below — the Prometheus TSDB is not one of them and has no off-cluster copy. See DEPLOYMENT.md "Recurring Snapshots (#63)". To opt a high-churn PVC out (e.g. a TSDB whose snapshot chain grows too large), label it into the `metrics` group instead — that group takes no snapshots and instead runs a nightly filesystem trim to reclaim freed blocks, which needs an ext4/XFS filesystem and the volume attached and mounted. See DEPLOYMENT.md "Excluded volumes: the metrics group (#101)".
