...
| `52_app_services.yml` | App-level secrets + DB bootstrap for auth-service/device-service/n8n | M7+ |
...
infra/
  ...
  playbooks/                # 00_bootstrap → 10_base → 20_k3s → 30_longhorn → 40_platform → 50_apps_infra → 51_homeassistant → 52_app_services
  roles/
...
cluster/
  ...
  apps/                     # Flux app configs (GitOps — managed by Flux)
    kustomization.yaml      # Lists device-service/, auth-service/, and n8n/
    ...
    n8n/                    # app manifests (deployment, service, pvc) reconciled by Flux
