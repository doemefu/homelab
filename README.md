...
| Restic restore test | Off-cluster backup target (Longhorn BackupTarget on Cloudflare R2) + restic datastore fix implemented via #64 — see [DEPLOYMENT.md](DEPLOYMENT.md) "Off-cluster backups (Longhorn BackupTarget)" | 🚧 Implemented — restore test pending rollout |
